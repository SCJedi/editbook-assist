...
import { getStreetColors, refreshStreetsList } from './streets.js';
...
function initCellClickHandlers() {
  document.querySelectorAll('.shelf-zoom-row .addr-block.clickable').forEach(block => {
    block.addEventListener('click', (e) => {
      // Don't open if clicking a drag handle
      ...
      openDetail(addrId, () => {
        // Re-render on close to reflect any annotation changes
        if (zoomedShelf !== null) {
          zoomIntoShelf(zoomedShelf);
        } else {
          renderSection();
...
function zoomAddrBlockHTML(seg) {
  ...
  if (addr.streetName) {
    ...
    if (color) {
      inner += `<div class="street-color-band" style="background:${color}"></div>`;
    }
  }
...
function zoomStreetGroupHTML(seg) {
  ...
  for (let i = 0; i < seg.addresses.length; i++) {
    ...
    if (color) {
      block += `<div class="street-color-band" style="background:${color}"></div>`;
    }
