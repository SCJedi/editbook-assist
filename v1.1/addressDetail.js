...
function populateHighlightControls(highlight) {
  ...
  if (highlightBgColor) highlightBgColor.value = highlight?.bgColor || '#000000';
  ...
  if (highlightBgColor && !highlightBgColor.previousElementSibling?.classList.contains('color-palette')) {
    ...
    const currentColor = (highlight?.bgColor || '#000000').toLowerCase();
...
function autoSaveHighlight() {
  ...
  const bgColor = highlightBgColor?.value || '#000000';
  const title = highlightTitle?.value || '';

  if (!title && bgColor === '#000000') {
    return;
  }
...
function buildTagFormHTML(tag) {
  ...
  const fontColor = tag?.fontColor || '#daa520';
  const bgColor = tag?.bgColor || '#3d2e00';
...
function handleTagFormSave() {
  ...
  const fontColor = document.getElementById('tagFormFontColor')?.value || '#daa520';
  const bgColor = document.getElementById('tagFormBgColor')?.value || '#3d2e00';
