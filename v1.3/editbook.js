...
function renderTable() {
  ...
  addresses.forEach((addr, idx) => {
    ...
    seqInput.dataset.originalSeq = String(idx + 1);
    seqInput.min = '1';
    seqInput.addEventListener('input', () => {
      if (seqInput.value !== seqInput.dataset.originalSeq) {
        seqInput.classList.add('seq-changed');
      } else {
        seqInput.classList.remove('seq-changed');
      }
    });
...
function handleImportConfirm() {
  ...
  const count = parseAndImportText(text);
  ...
  if (count > 0) {
    showStatus(`IMPORTED ${count} ADDRESSES`);
  }
}
...
// Apply sequence handler
function handleApplySequence() {
  const inputs = document.querySelectorAll('#editBookTable tbody .seq-input');
  if (inputs.length === 0) {
    showStatus('NO ADDRESSES TO REORDER');
    return;
  }
  ...
  inputs.forEach((input, idx) => {
    ...
    if (isNaN(val) || val < 1 || val > n) {
      input.classList.add('seq-invalid');
      hasInvalid = true;
    } else {
      input.classList.remove('seq-invalid');
    }
    seqValues.push({ idx, val });
  });

  if (hasInvalid) {
    showStatus(`INVALID SEQUENCE — ALL VALUES MUST BE INTEGERS FROM 1 TO ${n}`);
    return;
  }

  // Check for duplicates
  const seen = new Set();
  for (const { val } of seqValues) {
    if (seen.has(val)) {
      showStatus('INVALID SEQUENCE — DUPLICATE VALUES FOUND');
      // Mark duplicates
      const counts = {};
      seqValues.forEach(sv => { counts[sv.val] = (counts[sv.val] || 0) + 1; });
      inputs.forEach((input, i) => {
        if (counts[seqValues[i].val] > 1) {
          input.classList.add('seq-invalid');
        }
      });
      return;
  ...
  // Check for gaps (must be 1..N with no gaps)
  const sorted = [...seqValues].map(sv => sv.val).sort((a, b) => a - b);
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i] !== i + 1) {
      showStatus(`INVALID SEQUENCE — MUST BE 1 THROUGH ${n} WITH NO GAPS`);
      return;
    }
  }

  // Valid — reorder addresses array to match new sequence numbers
  const reordered = new Array(n);
  seqValues.forEach(({ idx, val }) => {
    reordered[val - 1] = addresses[idx];
  });

  addresses.splice(0, addresses.length, ...reordered);
  saveAddresses();
  renderTable();
  showStatus(`SEQUENCE APPLIED — ${n} ADDRESSES REORDERED`);
}
...
export function initEditBook(slots) {
  ...
  if (btnCsvUpload && csvFileInput) {
    ...
    csvFileInput.addEventListener('change', () => {
      ...
      reader.onload = (e) => {
        ...
        const count = parseAndImportText(text);
        if (count > 0) {
          showStatus(`IMPORTED ${count} ADDRESSES FROM FILE`);
        }
  ...
  const btnApplySequence = document.getElementById('btnApplySequence');
  if (btnApplySequence) btnApplySequence.addEventListener('click', handleApplySequence);
